...
在`PicReader::read`中利用`QPdfDocument::render`获取对应的`QImage`，并且交由ocr接口进行读取
### 3. JPG/PNG录入
...
## IV 人岗匹配
在 `MatchingRateAnalysis::singleCvAnalysis` 函数中，对每一条岗位进行匹配判断，首先是判断学历，工作年限信息，然后判断词频的匹配度，在达标后将岗位名称加入匹配数组，在后续整合为QString输出。  
```c++
for (auto it = demandList.begin(); it != demandList.end(); ++it) {
        ...
        if (edu2Enum(jobInfo["edu"]) <= cvEduInt || 
        jobInfo["edu"].isEmpty()) {
